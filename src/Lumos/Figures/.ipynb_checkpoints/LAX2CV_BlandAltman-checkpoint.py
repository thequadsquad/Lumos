...
class LAX2CV_BlandAltman(Visualization):
    def visualize(self, view, evals1, evals2, with_swarmplot=True):
        ...
        cps = [view.clinical_parameters[cpn] for cpn in ['2CV_LAESAREA', '2CV_LAEDAREA', 'LAESP_2CV', 'LAEDP_2CV']]
        cpname2diff = dict()
        ...
        yabs_max = max([max(axes[0,0].get_ylim(),key=abs), max(axes[0,1].get_ylim(),key=abs)])+2
        for ax in [axes[0,0], axes[0,1]]: ax.set_ylim(ymin=-yabs_max, ymax=yabs_max)
        yabs_max = max([max(axes[1,0].get_ylim(),key=abs), max(axes[1,1].get_ylim(),key=abs)])+2
        for ax in [axes[1,0], axes[1,1]]: ax.set_ylim(ymin=-yabs_max, ymax=yabs_max)


        # Tolerance Range
        ax = axes[2,0]
        ax.set_title('LAEDArea Equivalence Test', fontsize=26)
        cr = view.clinical_parameters['2CV_LAEDAREA']
        ax.axhspan(-cr.tol_range, cr.tol_range, facecolor='0.6', alpha=0.5)
        alpha = 0.5 if with_swarmplot else 0.0
        sns.swarmplot(ax=ax, y=cpname2diff['2CV_LAEDAREA'], palette=sns.color_palette("Blues")[4:],
                      dodge=True,size=5,alpha=alpha)
        ci = 1.96 * np.std(cpname2diff['2CV_LAEDAREA']) / np.sqrt(len(cpname2diff['2CV_LAEDAREA']))
        sns.pointplot(ax=ax, y=cpname2diff['2CV_LAEDAREA'], errorbar=("ci", 95), color='r')
        maxx = np.max([np.abs(np.min(cpname2diff['2CV_LAEDAREA'])), 
                       np.abs(np.max(cpname2diff['2CV_LAEDAREA'])),
                       np.abs(np.mean(cpname2diff['2CV_LAEDAREA'])-ci), 
                       np.abs(np.mean(cpname2diff['2CV_LAEDAREA'])+ci), 
                       cr.tol_range])
        ...
        for eva1, eva2 in zip(evals1, evals2):
            esp, edp = view.clinical_parameters['LAESP_2CV'], view.clinical_parameters['LAEDP_2CV']
            for p_i, (p1,p2) in enumerate([(esp.get_val(eva1),esp.get_val(eva2)), (edp.get_val(eva1),edp.get_val(eva2))]):
                for d in range(eva1.nr_slices):
                    try: 
                        ...
                        if not anno1.has_contour('la') or not anno2.has_contour('la'): continue
                        dice_val = dice_metric.get_val(anno1.get_contour('la'), anno2.get_contour('la'))
        ...
        ax.set_title('Dice Values by Phase', fontsize=26)
