...
class LAX_CINE_4CV_View(View):
    def __init__(self):
        self.name = 'LAX CINE 4CV'
        self.contour_names = ['la', 'ra']#'lv_lax_endo', 'lv_lax_myo', 'rv_lax_endo', 
        self.cmap = 'gray'
        self.clinical_parameters =  [cr() for cr in [LAX_4CV_LAESPHASE, LAX_4CV_LAEDPHASE, LAX_4CV_RAESPHASE, 
                                                     LAX_4CV_RAEDPHASE, LAX_4CV_RAESAREA,  LAX_4CV_RAEDAREA,
                                                     LAX_4CV_LAESAREA,  LAX_4CV_LAEDAREA]]
        self.clinical_parameters = {cr.name: cr for cr in self.clinical_parameters}
        self.clinical_phases = {'la':  [LAX_4CV_LAESPHASE().name, LAX_4CV_LAEDPHASE().name],
                                'ra':  [LAX_4CV_RAESPHASE().name, LAX_4CV_RAEDPHASE().name],
                                'all': [LAX_4CV_LAESPHASE().name, LAX_4CV_LAEDPHASE().name, 
                                        LAX_4CV_RAESPHASE().name, LAX_4CV_RAEDPHASE().name]}
        ...
        # attach CRs
        """
        self.clinical_parameters =  [LAX_4CV_RAESAREA,   LAX_4CV_RAEDAREA,
                                  LAX_4CV_LAESAREA,   LAX_4CV_LAEDAREA,
    ...
    def store_information(self, evals1, evals2, path, icon_path, storage_version=0):
        if storage_version >= 0:
            ...
            try:
                cr_overview_figure = LAX4CV_BlandAltman()
        ...
        if storage_version>=0:
            try:
                ...
                pdf.set_text('Fig. 1 Area Differences for LA & RA: The first row presents Bland-Altman plots for the 4CV RA areas in ES and ED. The second row shows BA plots for the 4CV LA areas. The third row shows 2CV LA areas. The last row contains Dice value boxplots per contour on the left and Hausdorff distance boxplots on the right. Legend: ES: End-systole, ED: End-diastole, CV: Chamber View, LA: Left Atrium, RA: Right Atrium, Dice: Dice similarity coefficient', 10, 247)
            except: print(traceback.print_exc())
