...
from shapely.geometry import Polygon, MultiPolygon, LineString, GeometryCollection, Point, MultiPoint, shape, box
...
def to_mask_pct(geo, height, width): 
    ...
    try: 
        if geo.geom_type == 'Polygon':    
            alltouched = features.rasterize([geo], size, all_touched=True)
            exterior = features.rasterize([geo.exterior, *geo.interiors], size, all_touched=True)
        ...
        if geo.geom_type == 'MultiPolygon':   
            for g in range(0, len(geo.geoms)):
                ...
                alltouched = features.rasterize([geo_multi], size, all_touched=True)
                exterior = features.rasterize([geo_multi.exterior, *geo_multi.interiors], size, all_touched=True)
                pctcover = (alltouched - exterior).astype(np.float32)
                for r, c in zip(*np.where(exterior == 1)):
                    # Find cell bounds, from rasterio DatasetReader.window_bounds
                    window = ((r, r+1), (c, c+1))
