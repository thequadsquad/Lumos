...
############################
#   LGE Specific Metrics   #
############################

class ThreshDiffMetric(Metric):
    ...
    def set_information(self):
        self.name = 'NumberDiff'
        self.unit = '[#]'

    @Metrics_exception_handler
    def get_val(self, eval1, eval2, slice_nr, phase_nr1, phase_nr2, string=False):
        """returns thresh difference 

        Args:
            eval1 (LazyLuna.eval): evaluation of first method for case comparison
            eval2 (LazyLuna.eval): evaluation of second method for case comparison
            slice_nr (int):        slice number (same for both comparison objects)
            phase_nr1 (int):       phase of first comparison object 
            phase_nr2 (int):       phase of second comparison object 
            string (bool):         return string of float with 2 decimal places if True
            
        Returns:
            float | str: Number distance of Threshold1 and Threshold2
        """
        thresh1 = eval1.get_threshold(slice_nr, phase_nr1)
        thresh2 = eval2.get_threshold(slice_nr, phase_nr2)
        dist    = thresh1 - thresh2
        return "{:.2f}".format(dist) if string else dist


class AvgAreaPerThreshMetric(Metric):
    """number distance calculation
    ...
    def set_information(self):
        self.name = 'AvgAreaPerThresholdstep'
        self.unit = '[cm2]'

    @Metrics_exception_handler
    def get_val(self, eval1, eval2, slice_nr, phase_nr1, phase_nr2, dcm=None, string=False):
        """returns average area differance per threshstep 

        Note:
            - shows influence of threshold difference on the area difference
            - high: big area change per threshold step --> small threshold difference has big influence on area
            - low:  small area change per threshold step --> threshold difference has small influence on area
        Args:
            eval1 (LazyLuna.eval): evaluation of first method for case comparison
            eval2 (LazyLuna.eval): evaluation of second method for case comparison
            slice_nr (int):        slice number (same for both comparison objects)
            phase_nr1 (int):       phase of first comparison object 
            phase_nr2 (int):       phase of second comparison object 
            dcm (dicom.pydicom):   Dicom object
            string (bool):         return string of float with 2 decimal places if True
        ...
        """
        pw, ph  = dcm.PixelSpacing
        anno1   = eval1.get_anno(slice_nr, phase_nr1)
        geo1    = anno1.get_contour('lv_scar')
        anno2   = eval2.get_anno(slice_nr, phase_nr2)
        geo2    = anno2.get_contour('lv_scar')
        m       = (geo1.area - geo2.area) * (pw*ph) 
        ...
        try:
            area_per_thresh = m/dist
        except ZeroDivisionError :
            area_per_thresh = np.nan
        
        return "{:.4f}".format(area_per_thresh) if string else area_per_thresh
