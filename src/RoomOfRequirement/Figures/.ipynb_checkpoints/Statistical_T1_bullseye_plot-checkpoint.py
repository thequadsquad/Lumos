...
class Statistical_T1_bullseye_plot(Visualization):
    def set_values(self, view, evals, canvas):
        self.view   = view
        self.evals  = evals
        self.canvas = canvas
    ...
    def visualize(self, segBold=[], minv=None, maxv=None):
        ...
        """
        self.clear()
        taskname = self.evals[0].taskname
        ...
        for eva in self.evals:
            aha = eva.aha_model
            means = [aha[n][0] for n in names[0]]+[aha[n][0] for n in names[1]]+[aha[n][0] for n in names[2]]
            stds  = [aha[n][1] for n in names[0]]+[aha[n][1] for n in names[1]]+[aha[n][1] for n in names[2]]
        ...
        ax.set_title(taskname + ' Average AHA Model (mean±std [ms] (n))')
    ...
    def store(self, storepath, figurename='_Averages_AHA_model.png'):
        self.savefig(os.path.join(storepath, self.evals[0].taskname+figurename), dpi=300, facecolor="#FFFFFF")
        return os.path.join(storepath, self.evals[0].taskname+figurename)