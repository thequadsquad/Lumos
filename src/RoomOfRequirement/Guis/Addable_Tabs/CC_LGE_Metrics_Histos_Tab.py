...
class CC_LGE_Metrics_Histos_Tab(QWidget):
    def __init__(self):
        super().__init__()
        self.name = 'LGE: Metric Table and Figure and Histograms'
    ...
    def make_tab(self, gui, view, eval1, eval2):
        ...
        #hinzugefügt if else Block:
        if type(view) in [SAX_LGE_View]:
            self.metrics_table = SAX_LGE_CC_Metrics_Table()
        else:
            self.metrics_table = Metrics_Table()
        ...
        self.annotation_histo_comparison_figure = LGE_Annotation_Histo_Comparison()
        self.annotation_histo_comparison_canvas = FigureCanvas(self.annotation_histo_comparison_figure)
        self.annotation_histo_comparison_figure.set_values(view, self.annotation_histo_comparison_canvas, eval1, eval2)
        self.annotation_histo_comparison_figure.visualize(0, 0, 0, view.contour_names[0])
        self.annotation_histo_comparison_canvas.mpl_connect('key_press_event', self.annotation_histo_comparison_figure.keyPressEvent)
        self.annotation_histo_comparison_canvas.setFocusPolicy(QtCore.Qt.FocusPolicy.ClickFocus)
        self.annotation_histo_comparison_canvas.setFocus()
        self.annotation_histo_comparison_toolbar = NavigationToolbar(self.annotation_histo_comparison_canvas, gui)
        layout.addWidget(self.annotation_histo_comparison_canvas,  11,0, 1,10)
        layout.addWidget(self.annotation_histo_comparison_toolbar, 12,0, 1,10)
    ...
    def recalculate_figure(self):
        ...
        try:
            #annocomparison
            p1, p2 = self.annotation_histo_comparison_figure.p1, self.annotation_histo_comparison_figure.p2
            d = self.annotation_histo_comparison_figure.slice_nr
            if self.annotation_histo_comparison_figure.clinical_phases and cont_name!=self.annotation_histo_comparison_figure.contour_name:
                phase_classes = sorted(list(self.view.clinical_phases[cont_name]))
                self.annotation_histo_comparison_figure.clinical_p = phase_classes[0]
                
            self.annotation_histo_comparison_figure.visualize(d, p1, p2, cont_name)
